...
class WikiVote:
    def __init__(self):
        self.data_path = "C:\\SHIFAN\\PythonProgram\\parallel-pll\\datasets\\Wiki-Vote.txt"
        ...
        self.first = []
        self.degree = []
        if os.path.exists(os.path.join(self.cache_path, "wiki_vote_edges")) is False:
            self.read_graph()
            utils.save_object(self.edges, os.path.join(self.cache_path, "wiki_vote_edges"))
            utils.save_object(self.first, os.path.join(self.cache_path, "wiki_vote_first"))
            utils.save_object(self.degree, os.path.join(self.cache_path, "wiki_vote_degree"))
        else:
            self.edges = utils.load_object(os.path.join(self.cache_path, "wiki_vote_edges"))
            self.first = utils.load_object(os.path.join(self.cache_path, "wiki_vote_first"))
            self.degree = utils.load_object(os.path.join(self.cache_path, "wiki_vote_degree"))
    ...
    def read_dict(self):
        path = os.path.join(self.cache_path, "wiki_vote_dict")
